...
const useFetch = ({ fetchDetail }) => {
    ...
    const loadMorePokemon = async () => {
        ...
        if (loadPokemon.includes("?limit=")) {
            ...
            await axios
                ...
                .then((res) => {
                    setCurrentPokemonType(res.data.types)
                    setCurrentPokemonImage(res.data.sprites.front_default)
                    setCurrentPokemonId(res.data.id)
                    setCurrentPokemonAbilities(res.data.abilities)
                    setCurrentPokemonStats(res.data.stats)
                    setLoading(false);
                }
                )
        } else {
    ...
    return { currentPokemon, totalPokemon, loading, currentPokemonType, currentPokemonImage, currentPokemonId, currentPokemonAbilities, currentPokemonStats }
}
